...
class SettingsMeta(SingletonMeta):
    ...
    def _set_figure_params(  # noqa: PLR0913
        ...
        *,
        ...
        ----------
        metacell
            Init default values for :obj:`matplotlib.rcParams` suited for metacell.
        dpi
        ...
        fontsize
            Set the fontsize for several `rcParams` entries. Ignored if `metacell=False`.
        figsize
            Set `rcParams['figure.figsize']`.
        color_map
            Convenience method for setting the default color map. Ignored if `metacell=False`.
        format
        ...
        if facecolor is not None:
            ...
            rcParams["axes.facecolor"] = facecolor
        if metacell:
            from ..plotting._rcmod import set_rcParams_metacell

            set_rcParams_metacell(fontsize=fontsize, color_map=color_map)
