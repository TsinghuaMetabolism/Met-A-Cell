...
def extract_mz_intensity_from_scm_events(mdata, offset=1, ppm_threshold=10):
    ...
    scm_event_idx = np.array(mdata.scm_events_index[mdata.main_cell_marker])
...
def correct_mz_to_reference(metab_anno: pd.DataFrame,
                            mz_ref: float,
                            mz_target: float = 760.5851,
                            mz_col: str = 'mz') -> pd.DataFrame:
    """
    Perform ppm correction based on the direction rule:
      - If mz_ref < mz_target (default 760.5851), subtract the ppm from the entire mz column (scale down)
      - If mz_ref > mz_target, add the ppm to the entire mz column (scale up)
    This function returns a corrected copy and does not modify the original DataFrame.

    Parameters
    ----------
    metab_anno : DataFrame
        Must contain a column with m/z values.
    mz_ref : float
        Reference m/z value (e.g., 760.5803).
    mz_target : float, optional
        Fixed target m/z value (default 760.5851).
    mz_col : str, optional
        Column name containing m/z values (default 'mz').

    Returns
    -------
    corrected_df : DataFrame
        Corrected DataFrame with m/z values adjusted according to ppm difference.
        A new column 'ppm_shift' is added to record the applied ppm per feature.
    """
    if mz_col not in metab_anno.columns:
        raise KeyError(f"'{mz_col}' column not found in DataFrame.")

    # Calculate ppm magnitude (absolute value)
    ppm_mag = abs(mz_target - mz_ref) / mz_ref * 1e6

    # Direction rule: if ref < target → subtract; if ref > target → add
    direction = -1 if mz_ref < mz_target else 1  # -1 = subtract, +1 = add
    scale = 1 + direction * (ppm_mag / 1e6)

    corrected_df = metab_anno.copy()
    old_mz = corrected_df[mz_col].astype(float)
    new_mz = old_mz * scale

    corrected_df[mz_col] = new_mz
