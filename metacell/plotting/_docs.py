...
doc_madata_color_etc = """\
madata
    Annotated data matrix.
color
    Keys for annotations of observations/cells or variables/metabolites, e.g.,
...
use_raw
    Use `.raw` attribute of `madata` for coloring with metabolites. If `None`,
    defaults to `True` if `layer` isn't provided and `madata.raw` is present.
layer
...
color_map
    ...
    The default `color_map` can be set using :func:`~metacell.set_figure_params`.
palette
    ...
    variable already has colors stored in `madata.uns["{var}_colors"]`.
    If provided, values of `madata.uns["{var}_colors"]` will be set.\
"""
...
frameon
    ...
    :func:`~metacell.set_figure_params`, defaults to `True`.
title
...
vcenter
    ...
    Example: ``mc.pl.umap(adata, color='TREM2', vcenter='p50', cmap='RdBu_r')``\
"""
...
metabolite_annotation
    Column name in `.var` DataFrame that stores metabolite annotation.
...
doc_rank_metabolites_groups_plot_args = """\
adata
    Annotated data matrix.
groups
    The groups for which to show the metabolites ranking.
n_metabolites
    Number of metabolites to show. This can be a negative number to show for
    example the down regulated metabolites. eg: n_metabolites=-10. Is ignored if
    `metabolites_names` is passed.
metabolites_annotation
    Column name in `.var` DataFrame that stores gene symbols. By default `var_names`
    refer to the index column of the `.var` DataFrame. Setting this option allows
    alternative names to be used.
groupby
    The key of the observation grouping to consider. By default,
    the groupby is chosen from the rank metabolites groups parameter but
...
"""
# pl.dotplot, pl.matrixplot
doc_rank_metabolites_groups_values_to_plot = """\
values_to_plot
    Instead of the mean metabolites value, plot the values computed by `mc.rank_metabolites_groups`.
...
var_names
    Metabolites to plot. Sometimes is useful to pass a specific list of var names (e.g. metabolite)
    to check their fold changes or p-values, instead of the top/bottom genes. The
    var_names could be a dictionary or a list as in :func:`~mc.pl.dotplot` or
    :func:`~mc.pl.matrixplot`. See examples below.\
"""