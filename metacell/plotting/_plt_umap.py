...
def plt_umap_diy(
    ...
    if "X_umap" not in madata.obsm:
        raise ValueError("madata.obsm['X_umap'] 不存在，请先计算 UMAP。")

    obs = madata.obs.copy()
    umap = madata.obsm["X_umap"]
    if umap.shape[1] < 2:
        raise ValueError("X_umap 少于 2 维。")
    obs["UMAP1"] = umap[:, 0]
    obs["UMAP2"] = umap[:, 1]

    if color_by not in obs.columns:
        raise KeyError(f"{color_by!r} 不在 madata.obs 中。")

    color_series = obs[color_by]
    is_continuous = pd.api.types.is_numeric_dtype(color_series)

    # 自动 leiden -> Cluster 替换
    if color_by == 'leiden' and replace_map is None and not is_continuous:
        unique_vals = sorted(color_series.astype(str).unique(), key=lambda x: int(x))
        replace_map = {val: f"Cluster {i+1}" for i, val in enumerate(unique_vals)}

    if replace_map is not None and not is_continuous:
        color_series = color_series.astype(str).map(replace_map)

    obs = obs.assign(_color=color_series).copy()
    obs = obs[obs["_color"].notna()].copy()

    if include is not None:
        obs = obs[obs["_color"].isin(include)]
    if exclude is not None:
        obs = obs[~obs["_color"].isin(exclude)]

    plt.figure(figsize=figsize, dpi=dpi)

    if is_continuous:
        cmap = get_cmap(palette) if isinstance(palette, str) else get_cmap(None)
        sc = plt.scatter(
            obs["UMAP1"], obs["UMAP2"],
            c=obs["_color"].astype(float).values,
            cmap=cmap,
        ...
        # 自动排序 legend 标签
        cats = order if order is not None else sorted(pd.unique(obs["_color"]))

        if palette is None or isinstance(palette, str):
            base_cmap = get_cmap("tab10")
            color_list = [base_cmap(i % 10) for i in range(len(cats))]
            palette_dict = {c: color_list[i] for i, c in enumerate(cats)}
        else:
            palette_dict = dict(palette)

        for cls in cats:
            sub = obs[obs["_color"] == cls]
            if sub.empty:
                continue
            plt.scatter(
                sub["UMAP1"], sub["UMAP2"],
                s=point_size, alpha=alpha,
                c=[palette_dict.get(cls, "#cccccc")],
                label=str(cls),
                edgecolors=edgecolor, linewidths=linewidth
            )
    ...
    plt.title(title if title else "UMAP Scatter", fontsize=14, fontweight="bold", pad=12)
