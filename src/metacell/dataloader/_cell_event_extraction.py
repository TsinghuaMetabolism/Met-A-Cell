...
def extract_scm_events(mdata: scMetData, cell_marker: dict, main_cell_marker: str = None, result_path: str = None,
    ...
    for marker in cell_marker.keys():
        ...
        if marker != 'TIC':
            mz_intensity = extract_intensity(mdata, name=marker, mz=cell_marker[marker])
...
def extract_intensity(mdata: scMetData, name: str, mz: float) -> pd.DataFrame:
    """
    ...
    density_center, mz_intensity = double_scan_feature_integration(mdata.mz_data, mdata.intensity_data, mz)
