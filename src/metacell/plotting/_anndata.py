...
class VarGroups(NamedTuple):
    ...
    one for the var_names (eg metabolites) in positions 0-4 and other for positions 5-8
    """
...
def ranking(  # noqa: PLR0912, PLR0913
    ...
    *,
    ...
    if isinstance(keys, str) and indices is not None:
        ...
        scores = _to_2d_array(getattr(adata, attr)[keys])
    else:
        scores = _to_2d_array(getattr(adata, attr)[dictionary][keys])
